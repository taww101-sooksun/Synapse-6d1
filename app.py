...
# --- 0. ตั้งค่าสมอง AI GEMINI (ฉบับสืบหาความจริง) ---
try:
    genai.configure(api_key=st.secrets["GOOGLE_API_KEY"])
    
    # บรรทัดนี้จะไปดึงชื่อรุ่นทั้งหมดที่ "คีย์ของหัวหน้า" มีสิทธิ์ใช้มาโชว์ครับ
    available_models = [m.name for m in genai.list_models() if 'generateContent' in m.supported_generation_methods]
    st.write(f"🔍 รุ่นที่หัวหน้าใช้ได้ตอนนี้คือ: {available_models}")
    
    # ลองใช้ตัวแรกที่มันหาเจอไปเลยครับ จะได้ไม่พลาด
