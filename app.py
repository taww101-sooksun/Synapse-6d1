...
# 2. ส่วนรับข้อมูล (รองรับปี ค.ศ. 1960 - 2026 ตามสัจธรรมความจริง)
...
# A. วันในสัปดาห์ (จันทร์=1 ยัน อาทิตย์=7 เป๊ะ)
...
# C. ระบบแกะรหัส 12 นักษัตรใหม่ (ยึดตามเกณฑ์ของนาย: ชวด=1 ยัน ปีกุล=12)
thai_year = selected_date.year + 543

def get_exact_zodiac(year):
    # ลำดับตามเกณฑ์ของนาย: ชวด=1, ฉลู=2, ขาล=3, เถาะ=4, มะโรง=5, มะเส็ง=6, มะเมีย=7, มะแม=8, วอก=9, ระกา=10, จอ=11, ปีกุล=12
    zodiac_order = [
    ...
    # พ.ศ. 2503 คือปีชวด (อันดับ 1 ในลิสต์) สตาร์ทนับจากจุดนี้จะตรงกับปฏิทินหลวงร้อยปีทั้งหมด
    base_year = 2503
...
# E. สมการและตรรกะตามใบสั่งล่าสุดของนายเป๊ะๆ
# 1. ฐานรวม (วัน + เดือน + ปีนักษัตร)
base_sum = val_day + val_month + val_zodiac

# 2. หาค่าอัตราส่วนดวงจันทร์ (29.53 หารด้วย ค่ำประจำวัน)
if lunar_step != 0:
    lunar_ratio = 29.53 / lunar_step
else:
    lunar_ratio = 0

# 3. รวมฐานกับอัตราส่วนเข้าด้วยกัน
total_combined = base_sum + lunar_ratio

# 4. คูณทองคำจักรวาล 1.618 ปิดสมการสุทธิ
final_result = total_combined * 1.618


# 4. แสดงผลหน้าจอมือถือสไตล์ดาร์กนีออน (ลบคำซ้ำซ้อนออกตามภาพที่ส่งมา)
st.write("---")
...
with st.expander("📝 ขั้นตอนสมการการถอดรหัส"):
    ...
    **คำนวณตามขั้นตอนสมรรถนะ:**
    ...
    4. **คูณรหัสทองคำจักรวาล:** {total_combined:.2f} × 1.618 = **{final_result:.4f}**
    """)
...
st.caption("อยู่นิ่งๆ ไม่เจ็บตัว | ควบคุมความเที่ยงตรงโดยอาจารย์ต๊ะ 2026")
