...
with tabs[1]: # เรดาร์ (อัปเดตตามตำแหน่งที่พี่อยู่จริง)
    st.subheader(L["radar"])
    
    # 1. ดึงพิกัดล่าสุดที่พี่ "กดยืนยัน" มาจากหน้าแรก (ถ้าไม่มีให้ไปอ่อนนุชก่อน)
    # อ่อนนุชโดยประมาณคือ Lat: 13.7056, Lon: 100.6015
    current_lat = st.session_state.get('cur_lat', 13.7056)
    current_lon = st.session_state.get('cur_lon', 100.6015)
    
    st.write(f"🛰️ กำลังจับสัญญาณที่พิกัด: {current_lat}, {current_lon}")

    # 2. แสดงแผนที่ดาวเทียม Google Hybrid
    # ซูมระดับ 18 เพื่อให้เห็นหลังคาบ้านชัดๆ
    m = folium.Map(
    ...
    # 3. ปักหมุดสีแดงตรงตำแหน่งของพี่
    folium.Marker(
        [current_lat, current_lon], 
        popup="ตำแหน่งของคุณ (อ่อนนุช)",
